...
function saveSettings(partial) {
  try {
    ...
    // Se partial tiver totalGold ou permanentUpgrades, usamos eles
    // Se não, tentamos pegar das variáveis globais se existirem
...
window.mergeCloudSave = (cloudData) => {
  if(cloudData) {
    ...
    if (cloudData.permanentUpgrades) permanentUpgrades = { ...permanentUpgrades, ...cloudData.permanentUpgrades };
    
    // Atualiza a interface da loja do Mercado Negro imediatamente
    if(typeof renderShop === 'function') renderShop();
    
    // Mostra aviso de Download Cloud se a função existir
...
window.forcePushLocalToCloud = () => {
    saveSettings({}); // Um save em vazio força a leitura das variáveis globais e dispara o Push do Firebase!
};

let totalGold = 0;       // Ouro total acumulado (banco)
let totalRareStones = 0; // Pedras raras acumuladas (banco)
let permanentUpgrades = { maxHp: 0, damage: 0, goldGain: 0, regen: 0, masteredUpgrades: [] };
...
if (saved) {
  ...
  if (saved.permanentUpgrades) permanentUpgrades = { ...permanentUpgrades, ...saved.permanentUpgrades };
}
...
const particles = [];
const gems = [];
const coins = []; // Array para as moedas
const hearts = [];
...
function pickUpgrades(count = 3, forcedIds = []) {
...
function openUpgradeScreen(isReroll = false) {
  ...
  if (!isReroll) {
    ...
    const upgrades = pickUpgrades(3, lockedIds);
    nextLevelLockedIds = [];
    ...
    let newUpgrades = [];
    if (slotsNeeded > 0) {
      const candidates = pickUpgrades(10, []);
...
function renderUpgradeCards() {
  ...
  const pickCost = freeUpgrades > 0 ? 0 : upgradeCost;
...
function draw() {
  ...
  if (sprites.bg_space && sprites.bg_space.complete && sprites.bg_space.naturalWidth) {
    ...
    ctx.restore();
    // Contraste para personagens brilharem mais
  ...
  if (playerSprite && playerSprite.complete) {
    ...
    ctx.rotate(angle + Math.PI / 2);
    const sSize = player.radius * 3.4; // Um pouco maior que o raio pra cobrir colisões
...
navBtns.forEach(btn => {
  btn.addEventListener('click', (e) => {
    ...
    // Modo gambiarra pra pegar a pane certa (nav-btn-play -> menu-screen, nav-btn-shop -> shop-screen, nav-btn-settings -> settings-screen)
    let targetId = 'menu-screen';
    if(e.target.id === 'nav-btn-shop') {
      targetId = 'shop-screen';
      renderShop(); // renderiza qdo clica
    }
    if(e.target.id === 'nav-btn-settings') {
...
// Event Listener Iniciar Partida
const btnPlayNova = document.getElementById('btn-play');
if (btnPlayNova) {
  btnPlayNova.addEventListener('click', () => {
    if(typeof Audio !== 'undefined' && Audio.sfxClick) Audio.sfxClick();
    startGame();
  });
}

// Botões antigos de voltar pro menu que ainda estiverem flutuando em outras sub-telas
const btnAbandona = document.getElementById('btn-back-menu');
if(btnAbandona) {
   btnAbandona.addEventListener('click', () => window.location.reload());
}
...
function startGame() {
  ...
  score = 0; level = 1; xp = 0; xpNextLevel = 100; gold = 0; rareStones = 0;
  rerollCost = 20;
  upgradeCost = 15; // Resetar custo de upgrade para nova partida
  freeRerolls = 2; // Reseta rerolls grátis iniciais
  freeUpgrades = 1; // Reseta upgrade grátis inicial
  goldRushTimer = 0;
  powerups.length = 0;
  nextLevelLockedIds = [];
  upgradePickCount = {}; // Reseta contagem de upgrades da run anterior
  ...
  // reset skills ANTES de aplicar os upgrades da loja
  ...
  const bonusHp = (permanentUpgrades.maxHp || 0) * 10;
  player.maxHp = 100 + bonusHp;
  player.hp = player.maxHp;

  const bonusDmgPct = (permanentUpgrades.damage || 0) * 0.10;
  player.damage = 1 * (1 + bonusDmgPct);

  const bonusRegen = (permanentUpgrades.regen || 0) * 0.5;
  player.regen = 0 + bonusRegen;

  const bonusGold = (permanentUpgrades.goldGain || 0) * 0.10;
  ...
  // Aplicar upgrades do shop de maestria (permanentes por rank)
  ...
  // Aplicar habilidades permanentes via maestria
  const skillPermMap = {
...
function selectCharacter(weaponType) {
  ...
  // Atualiza visualização na UI da classe selecionada
  if (typeof characterCards !== 'undefined') {
    characterCards.forEach(c => {
      if (c.getAttribute('data-weapon') === weaponType) {
...
if (typeof selectCharacterBtns !== 'undefined') {
...
// Inicializa no DOM a seleção do personagem inicial se ouver algo
